...
$forum_ID = ! empty( $args['forum_ID'] ) ? $args['forum_ID'] : 0;
...
if ( ! empty( $post_ID ) ) :
	...
	if ( post_type_supports( get_post_type(), 'comments' ) ) {
		if ('forum' === get_post_type() && !empty($post_forum_ID = get_field('forum'))) {
			$comments_count = wp_count_comments( $post_ID );
			$reply_text = ($comments_count->approved === 1) ? 'Reply' : 'Replies';
			$post_meta[] = ['icon-chat-20', sprintf( _n( '%s Reply', '%s Replies', $comments_count->approved, 'weadapt' ), number_format_i18n( $comments_count->approved ) )];
		} else {
			$comments_count = wp_count_comments( $post_ID );
			$reply_text = ($comments_count->approved === 1) ? 'Comment' : 'Comments';
			$post_meta[] = ['icon-chat-20', sprintf( _n( '%s %s', '%s %s', $comments_count->approved, 'weadapt' ), number_format_i18n( $comments_count->approved ), $reply_text )];
		}
	}
...
<article class="cpt-list-item blog-list-item">
	...
	<div class="cpt-list-item__content">
		<?php
			...
			if ( ! empty( $post_forum_ID = get_field( 'forum' ) ) ) :
				?><div class="cpt-list-item__actions"><?php
					echo get_button( [
						'url' => get_permalink($post_ID) . '#reply',
						'title'  => __( 'Reply', 'weadapt' ),
						'target' => '',
					] );
				?></div><?php
			endif;
