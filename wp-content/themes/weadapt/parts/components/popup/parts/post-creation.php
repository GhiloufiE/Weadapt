...
<div class="popup__content sidebar-popup" aria-labelledby="post-creation" aria-hidden="true" id="post-creation-popup">
    <?php if (is_user_logged_in()) : ?>
        <form id="post-creation-form" method="post">
            ...
            <div class="form-group theme-select-wrap">
...
<script>
    function selectForum(forumId) {
        document.getElementById('selected-forum').value = forumId;
        console.log('Selected forum ID:', forumId);
    }
    document.addEventListener('DOMContentLoaded', function() {
        const forumSearchWrap = document.querySelector('.theme-select-wrap');
        const pageSourceField = document.getElementById('page-source-field');
        const popupButton = document.querySelector('[data-popup="post-creation"]');

        if (popupButton) {
            popupButton.addEventListener('click', function() {
                pageSourceField.value = 'connect-page';
                toggleForumField();
            });
        }

        function toggleForumField() {
            if (pageSourceField && pageSourceField.value === 'connect-page') {
