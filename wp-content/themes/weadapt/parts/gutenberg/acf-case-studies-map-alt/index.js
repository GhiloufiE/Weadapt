/* global vars, googleMapsVarsAltAlt, markerData, google, markerClusterer */
...
const initCustomSelect = function (selectWrap, updateMap) {
	...
	const customSelectTheme = selectWrap.querySelector('.theme-select');
	const customSelectType = selectWrap.querySelector('.post-type-select');
	...
	new SelectPure(customSelectTheme, {
		options,
	...
	new SelectPure(customSelectType, {
		options,
		multiple: select.multiple,
		autocomplete: true,
		value: selectedOptions,
	...
	if (markerNodes) {
		markerNodes.forEach(markerNode => {
			...
			marker.addListener('click', () => {
				...
				window.history.pushState({}, '', `${markerNode.dataset.url}/`);
	...
	const renderer = {
		render: ({ count, position }) => {
			const clustererSvg = [
				...
				`<circle cx="20" cy="20" r="15" fill="${googleMapsVarsAlt.markerColor}" stroke="${googleMapsVarsAlt.markerBgColor}" stroke-width="1" />`,
				'</svg>',
			].join('\n');
			...
			return new google.maps.Marker({
				...
				label: {
					text: String(count),
					color: googleMapsVarsAlt.markerBgColor,
					fontSize: '13px',
				},
				position,
	...
	const rendererCip = {
		render: ({ count, position }) => {
			const clustererSvg = [
				'<?xml version="1.0" encoding="UTF-8"?>',
				'<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40">',
				`<circle cx="20" cy="20" r="15" fill="${googleMapsVarsAlt.markerCipBgColor}" stroke="${googleMapsVarsAlt.markerCipColor}" stroke-width="1" />`,
				'</svg>',
			].join('\n');
			const clustererImage = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(clustererSvg)}`;
			...
			return new google.maps.Marker({
				...
				label: {
					text: String(count),
					color: googleMapsVarsAlt.markerCipColor,
					fontSize: '13px',
				},
				position,
				zIndex: Number(google.maps.Marker.MAX_ZINDEX) + count,
...
const mapFilter = function (formNode, map, markers, markerClusters) {
	...
	// Update URL
	window.history.pushState({}, '', `${markerData.currentUrl}?search=${formData.get('search')}&filter=${formData.get('theme_network')}&type=${formData.get('post_type')}`);

	formNode.classList.add('loading');

	fetch(vars.restSearchCaseStudySolutionMarkerstUrl, {
		method: 'POST',
		body: formData,
	})
		.then(response => response.text())
...
const mapBlockNodes = document.querySelectorAll('.case-studies-map-alt');
