...
function theme_publish_post( $post ) {
	...
	$post_ID = $post->ID;
	// Docx comment #4: when content is published all contributors are notified.
	$contributors = get_field( 'people_contributors', $post_ID );

	if ( ! empty( $contributors ) ) {
		$subject = sprintf( __( 'Your %s has been published on %s', 'weadapt' ),
	...
	// Docx comment #5: when content is published Editors of theme/network/microsite and weADAPT Admin are notified.
	...
	if ( ! empty( $related_themes_networks = get_field( 'relevant_themes_networks', $post_ID ) ) ) {
		...
		if ( ! empty( $relevant_users ) ) {
			$subject = sprintf( __( 'Content has been published on %s which is related to your theme / network', 'weadapt' ),
				get_bloginfo( 'name' )
			);
			...
			if ( ! empty( $post_author ) ) {
				$message = sprintf( __( 'Content has been published by <a href="%s">%s %s (%s)</a> on %s which is related to your theme / network.', 'weadapt' ),
				...
				$message = sprintf( __( 'Content has been published on %s which is related to your theme / network.', 'weadapt' ),
				get_bloginfo( 'name' )
				) . '<br><br>';
			}
			...
			$message .= sprintf( __( 'Content: %s', 'weadapt' ),
				esc_html( $post->post_title ),
			) . '<br>';
			$message .= sprintf( __( 'Summary: %s', 'weadapt' ),
