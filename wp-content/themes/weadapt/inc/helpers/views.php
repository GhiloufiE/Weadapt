...
function get_post_views( $post_ID = false, $abridged = true, $with_title = true ) {
	...
	// Get count
	$count  = intval( get_post_meta( $post_ID, '_views_count', true ) );
	$output = $count;

	// Abriged count
	if ( $abridged ) {
		$output = abridged_number( $count );
	}
 ...
 */
function set_post_views( $post_ID, $custom_count = 0 ) {
	...
	// Update Views
	$result = update_post_meta( $post_ID, '_views_count', $new_count );
