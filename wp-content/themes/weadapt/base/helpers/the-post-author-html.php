			...
			<div class="cpt-list-item__author-image">
				...
				<div class="cpt-list-item__author-info">
					...
					<?php else: ?>
						...
						<?php
						if (! empty($post_type)) {
							$article = use_an_or_a($post_type);

							switch (get_post_status()) {
								case 'pending':
									printf(__('Submitted %s ', 'weadapt'), $article);
									break;

								case 'draft':
								case 'future':
								case 'private':
									printf(__('Saved %s ', 'weadapt'), $article);
									break;

								default:
									printf(__('Published %s ', 'weadapt'), $article);
									break;
							}
							...
							if ('forum' === $post_type) {
								...
								if (! empty($post_forum)) {
									$output = sprintf(
										' <a href="%s" class="cpt-list-item__link">%s</a>',
