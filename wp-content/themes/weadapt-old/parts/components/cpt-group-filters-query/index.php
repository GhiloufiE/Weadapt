<?php
	...
	$is_upcoming     = isset( $args['is_upcoming'] ) ? wp_validate_boolean( $args['is_upcoming'] ) : false;
	$base_url        = get_current_clean_url();

	$query_type     = isset( $args['query_type'] ) ? $args['query_type'] : 'wp_query';
	$args_for_query = apply_filters( 'args_for_groups_filter', $temp_args, $_GET, $query_type );
...
?>
<div class="cpt-query" data-idx="<?php echo $number; ?>" data-tab="<?php echo $tab; ?>" >
	<div class="cpt-filters<?php echo ! $show_filters ? ' hidden' : ''; ?>">
		<form class="cpt-filters__form">
             ...
             <div class="cpt-filters__terms<?php echo ! empty( $post_types = $args_for_query['post_types'] ) || ($show_categories && ! empty( $categories = $args_for_query['categories'] ) ) ? '' : ' hidden'; ?>">
                <ul class="cpt-filters__list">
                    ...
                    <?php if ( ! empty( $post_types ) ) :
                        $args_for_query['tab'] = $_GET['tab'];
                        $args_for_query['selected_posts'] = $_GET['post_types'];
			...
			<input type="hidden" value="<?php echo $is_upcoming ? 'true' : 'false'; ?>" name="is_upcoming" />
		</form>
		...
		<?php
			...
			// WP_Query
				...
				if (
					...
					echo sprintf( '<span class="col-12 empty-result">%s</span>', __( $is_upcoming ? 'Coming soon!' : 'Nothing found.', 'weadapt' ) );
				}
				else {
					if ( $query->have_posts() ) {
