...
const videoInit = () => {
	const videoSections = document.querySelectorAll('.video-text');

	videoSections.forEach(video => {
		const videoTrigger = video.querySelector('.video-text__video-play');
		const videoPlayer = video.querySelector('iframe') ? video.querySelector('iframe') : video.querySelector('video');
		const videoParent = video.querySelector('.video-text__video');
