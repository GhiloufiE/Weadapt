...
$marker_data = [
	'currentUrl' => get_home_url( null, '/placemarks/alt/' ),
];
...
$type_value   = ! empty( $_GET['type'] ) ? $_GET['type'] : 'all';
?>
...
<section <?php echo $attr; ?>>
	...
	<div class="map__header container">
		...
		<form class="map__controls">
			...
			<?php
				$post_types = get_allowed_post_types( [ 'case-study', 'solutions-portal' ] );
				if( ! empty( $post_types ) && count( $post_types ) > 1 )  { ?>
					<div class="map__select">
						<select name="post_type">
							<option value="all" selected><?php _e( 'Any Resource...', 'weadapt' ); ?></option>
							<?php foreach( $post_types as $post_type ) : ?>
								<option value="<?php echo $post_type; ?>" <?php selected( $type_value, $post_type ); ?>>
									<?php if ($post_type == 'case-study' ) : ?>
										<?php _e( 'Case Study', 'weadapt' ); ?>
									<?php elseif ($post_type == 'solutions-portal' ) : ?>
										<?php _e( 'Solution', 'weadapt' ); ?>
									<?php endif; ?>
								</option>
							<?php endforeach; ?>
						</select>
						<div class="post-type-select"></div>
					</div>
				<?php }
			?>
		...
		<div class="map__wrap">
			<div class="acf-map">
				...
				if ( false === $locations ) {
					...
					if ( ! empty( $solutions->posts ) ) {
						foreach( $solutions->posts as $post_ID ) {
							$location = get_field( 'central_location', $post_ID );
							if ( ! empty( $location['lat'] ) && ! empty( $location['lng'] ) ) {
								$locations[$post_ID] = [
									'lat'   => $location['lat'],
									'lng'   => $location['lng'],
				...
				if ( ! empty( $locations ) ) {
					foreach ( $locations as $post_ID => $location ) {
						echo sprintf( '<div class="marker" data-id="%s" data-title="%s" data-lat="%s" data-lng="%s" data-slug="%s" data-url="%s"></div>',
							esc_attr( $post_ID ),
