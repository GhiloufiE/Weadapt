...
add_filter( 'walker_nav_menu_start_el', function( $item_output, $menu_item, $depth, $args ) {
	...
	if ( in_array( $args->theme_location, array( 'header-top-menu', 'header-main-menu', 'header-main-menu-logged-in' ) ) ) {
		...
		if ( 0 === $depth ) {
			...
			$menu_item_card     = get_field( 'card_menu', $menu_item );
...
<div class="container">
    <div class="row">
        <?php if ($menu_item_card) : ?>
            <?php foreach ($menu_item_card as $field_value) : ?>
                <div class="mega-menu__col mega-menu__col--card">
                    <div class="mega-menu__card">
                        <?php if (!empty($field_value['icon'])) : ?>
                            <div class="card-icon">
                                <img src="<?php echo $field_value['icon']; ?>" alt="Icon">
                            </div>
                        <?php endif; ?>

                        <?php if (!empty($field_value['title'])) : ?>
                            <h4 class="card-title"><?php echo $field_value['title']; ?></h4>
                        <?php endif; ?>

                        <?php if (!empty($field_value['description'])) : ?>
                            <p class="card-description"><?php echo $field_value['description']; ?></p>
                        <?php endif; ?>

                        <?php if (!empty($field_value['button'])) : ?>
                            <div class="card-button">
							<?php echo get_button( $field_value['button'] ); ?>                           
						 	</div>
                        <?php endif; ?>
                    </div>
                </div>
